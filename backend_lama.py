...
def fetch_macro_data(indicator: str, country: str, start_year: int = None, end_year: int = None, recent_years: int = 10):
    ...
    try:
        ...
        # --- PERBAIKAN: Melakukan melting (unpivot) data kolom tahun menjadi baris ---
        # Identifikasi kolom selain tahun (seperti 'economy' atau 'series')
        id_vars = [col for col in data.columns if not str(col).startswith('YR')]
        
        # Reshape data
        data = data.melt(id_vars=id_vars, var_name='Year', value_name='Value')
        
        # Bersihkan awalan 'YR' dari string tahun
        data['Year'] = data['Year'].astype(str).str.replace('YR', '')
        
        # Filter hanya menyisakan kolom yang dibutuhkan
        data = data[['Year', 'Value']]
        
        # Urutkan berdasarkan tahun agar data historisnya berurutan
